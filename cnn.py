...
def cnn_model(X_train, y_train, keep_prob, lamda, num_epochs = 450, minibatch_size = 16):
	...
	#full connection1(softmax layer)
...
if __name__ == "__main__":
	...
	print("载入训练集: " + str((time.strftime('%Y-%m-%d %H:%M:%S'))))
	X_train,y_train = load_dataset()
	print("开始训练: " + str((time.strftime('%Y-%m-%d %H:%M:%S'))))
	cnn_model(X_train, y_train, 0.8, 1e-4, num_epochs=500, minibatch_size=16)
