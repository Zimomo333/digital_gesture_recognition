...
def resize_img():
	...
	for filename in dirs:
		im = tf.gfile.FastGFile("picture//{}".format(filename), 'rb').read()
		...
		with tf.Session() as sess:
			...
			scipy.misc.imsave("resized//{}".format(filename),resized_im)
...
def image_to_h5():
	...
	for filename in dirs:
		...
		Y.append(label)
		im = Image.open("train//{}".format(filename)).convert('RGB')
